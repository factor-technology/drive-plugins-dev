...
The skill reads its guidance from your Drive connector, so that part is always
current. The plugin itself — which requests the skill answers and the
connector it wires up — changes less often, but it does change. Installing
from this marketplace gives you an update channel — but **turn auto-update
on, because it is off by default for marketplaces that aren't Anthropic's
own**:
